#|
Wow this assignment took way longer than I thought it would.
My code passes all of the provided test cases 
|#
...
(define (append-ld listdiff . stuff)
	(define (build-ld listdiff stuff) ; build list
		(if (null? stuff) ; exit case
			(car listdiff)
			(append (get-diff listdiff) (build-ld (car stuff) (cdr stuff))))) ; build list
...
(define (assq-ld obj alistdiff)
	(let [(head ; value to return
			(if (null? (cdr alistdiff)) ; was it built using listdiff fn?
				 	(car-ld alistdiff)
				 	(car alistdiff)))
...
(define (listdiff->list listdiff)
	(cond
		[(and (list? (car listdiff)) (null? (cdr listdiff))) (car listdiff)] ; if listdiff created using listdiff fn
		[(list? listdiff) listdiff] ; already valid
		[else (get-diff listdiff)] ; build it
		)
)
